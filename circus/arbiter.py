...
class Arbiter(object):
    ...
    def __init__(self, watchers, endpoint, pubsub_endpoint, check_delay=1.0,
        ...
        if self.statsd:
            ...
            stats_watcher = Watcher('circusd-stats', cmd=sys.executable, args=cmd, use_sockets=True,
                                    singleton=True,
        ...
        if httpd:
            ...
            # Adding the watcher
            httpd_watcher = Watcher('circushttpd', cmd=sys.executable, args=cmd, use_sockets=True,
                                    singleton=True,
                                    stdout_stream=self.stdout_stream,
                                    stderr_stream=self.stderr_stream,
                                    copy_env=True, copy_path=True,
        ...
        if plugins is not None:
            for plugin in plugins:
                ...
                plugin_cfg = dict(cmd=cmd, args=args, priority=1, singleton=True,
                                  stdout_stream=self.stdout_stream,
                                  stderr_stream=self.stderr_stream,
                                  copy_env=True, copy_path=True,
